...
# Pokemon Config file
POKEMON_CONFIG = "include/config/pokemon.h"
    ...
    with open(file, "r", encoding='utf8') as f:
        ...
        for raw_line in content:
            ...
            if "static const u16 s" in line:
                ...
                raw_species = line.replace("static const u16 s", "").replace(trailing, "").strip()
...
if __name__ == '__main__':
    ...
    # Table of enabled species
    species_enabled = None

    # Open the config file to check which species are enabled
    with open(f"include\\config\\species_enabled.h", "r", encoding='utf8') as f:
        species_enabled = cparser.parse_defines(f.readlines())


    # Loop over each generation
    for gen in range(1,10):
        with open(f"src\\data\\pokemon\\species_info\\gen_{gen}_families.h", "r", encoding='utf8') as f:
            info = cparser.parse_species_info(f.readlines())

            print(info)
            break

    with open("src\\data\\pokemon\\teachable_learnsets.h", "r", encoding='utf8') as f:
        ...
        for species in teachable:
            ...
            if species not in species_data:
                species_data[species] = {"name": format_species_name(species)}

            # Initialise species data
            species_data[species]["teachable"] = teachable_moves

    with open("src\\data\\pokemon\\egg_moves.h", "r", encoding='utf8') as f:
        ...
        for species in eggmoves:
            ...
            # Species not inserted, add it
            if species not in species_data:
                species_data[species] = {"name": format_species_name(species), "teachable": []}
            
            # Initialise species data
            species_data[species]["egg"] = teachable_moves

    with open("src\\data\\pokemon\\level_up_learnsets\\gen_9.h", "r", encoding='utf8') as f:
        ...
        for species in levelup:
            ...
            # Species not inserted, add it
            if species not in species_data:
                species_data[species] = {"name": format_species_name(species), "teachable": [], "egg": []}
            
            # Initialise species data
            species_data[species]["levelup"] = level_up_moves

    # print(species_data['Pikachu'])